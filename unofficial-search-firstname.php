...
$query = "SELECT lodger.ssn, lodger.lname, lodger.fname, lodger.mname, lodger.gender from lodger where lodger.ssn not in (SELECT lodger.ssn FROM lodger, official WHERE lodger.ssn = official.lodger_ssn UNION (SELECT lodger.ssn FROM lodger, reservation WHERE lodger.ssn = reservation.lodger_ssn)) AND lodger.fname LIKE '%$keyword%' ORDER BY lodger.lname";
